...
static double _get_error_prob_from_matrix(struct wmediumd *ctx, double snr,
					 unsigned int rate_idx, u32 freq,
					 ...
					 struct station *dst)
{
	if (dst == NULL) // dst is multicast. returned value will not be used.
		return 0.0;
    return 0.0;
//	return ctx->error_prob_matrix[ctx->num_stas * src->index + dst->index]; // FIXME!!!!
}
			  ...
			  struct station *dst, struct station *src)
{
    ...
    printf("ITU freq %f Path Loss %f Floor loss %f Num Floors %d PL %d\n", f, pL, param->lF, param->nFLOORS, PL);
...
/* Existing link is from -> to; copy to other dir */
...
int load_config(struct wmediumd *ctx, const char *file, const char *per_file, bool full_dynamic)
{
	...
	if (full_dynamic) {
		ctx->sta_array = malloc(sizeof(struct station *) * 5000); // FIXME!!!!
		ctx->num_stas = 0;
		ctx->intf = NULL;
		ctx->get_fading_signal = _get_fading_signal;
		ctx->fading_coefficient = 0;
        ctx->noise_threshold=0;
		ctx->move_stations = move_stations_donothing;
		ctx->snr_matrix = calloc(sizeof(int), 5000 * 5000); // FIXME!!!!
		ctx->per_matrix = NULL;
        ...
        ctx->calc_path_loss = calc_path_loss_itu;
        ctx->path_loss_param = param;
        ctx->get_error_prob = _get_error_prob_from_matrix;
		return 0;
	}
