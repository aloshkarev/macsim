...
static int init_netlink(struct wmediumd *ctx)
{
	...
	ctx->family_id = genl_ctrl_resolve(sock, "APRF_DRV");
	if (ctx->family_id < 0) {
		w_logf(ctx, LOG_ERR, "Family APRF_DRV not registered\n");
		return -1;
	}
...
{
	printf("eltex_wmediumd - a wireless medium simulator\n");
	printf("eltex_wmediumd [-h] [-V] [-s] [-l LOG_LVL] [-x FILE] -c FILE\n\n");
...
int main(int argc, char *argv[])
{
	...
	event_assign(&ev_cmd, ctx.ev_base, nl_socket_get_fd(ctx.sock), EV_READ | EV_PERSIST,
		  sock_event_cb, &ctx);
	...
	event_assign(&ev_timer, ctx.ev_base, ctx.timerfd, EV_READ | EV_PERSIST, timer_cb, &ctx);
